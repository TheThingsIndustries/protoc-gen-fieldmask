...
const transformer = `import (
	"strings"

	"github.com/gogo/protobuf/types"
)

func transformFieldMask(mask *types.FieldMask, prefix string) *types.FieldMask {
	var newPaths []string
	for _, path := range mask.Paths {
		if strings.HasPrefix(path, prefix+".") {
			newPaths = append(newPaths, strings.TrimPrefix(path, prefix+"."))
		}
	}
	return &types.FieldMask{Paths: newPaths}
}`
...
func (p *plugin) Generate(in *plugin_go.CodeGeneratorRequest) (*plugin_go.CodeGeneratorResponse, error) {
	...
	for dir := range dirs {
		...
		fmt.Fprintln(buf, fileHeader)
		fmt.Fprintln(buf)
		fmt.Fprintf(buf, "package %s", filepath.Base(dir))
		fmt.Fprintln(buf)
		fmt.Fprintln(buf)
		fmt.Fprintln(buf, transformer)
